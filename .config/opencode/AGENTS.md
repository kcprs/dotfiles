...
When making changes, hyper-focus exlusively on the main task given to you.

Unless explicitly mentioned as part of your task, **do not** do any of the following:

- Do not remove comments from code that you work on. This also applies to comments with tags like
  TODO or similar.
- Do not make improvements to code that does not need changing to achieve your main task.
- Do not remove code that seems unused, unless explicitly asked to do so.

If you come across code that you think should be changed or removed but the change is unrelated to
your main task, make a note of this in SESSION.md as described in the section below.
...
- Unless explicitly instructed, do not mention library internals in documentation comments. Focus on
  the purpose and usage of the documented item from the perspective of the library user.
